...
> **当前基线**：后端 `http://127.0.0.1:8000`，前端 `http://127.0.0.1:5173`。FHIR Adapter `http://127.0.0.1:8300`。前端代理 `5173 -> 8000`。接口总数、运行模式和生产边界见 [臻护-代码现状基线.md](臻护-代码现状基线.md)。本文按推荐业务入口组织，不将 `/v1/*` 兼容路径重复列为独立能力。
...
| 正式部署约定 | `http://127.0.0.1:8000`（由 Nginx/服务编排统一暴露） |
