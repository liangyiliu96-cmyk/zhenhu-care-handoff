...
> **当前本地运行基线（2026-07-28）**：浏览器唯一入口为 `http://127.0.0.1:5173`，Vite 将所有业务 API 代理到 `http://127.0.0.1:8001` 的 `inpatient-ward`。`8100`、`8200`、`8300` 仅供后端桥接服务使用；Docker 中 FastAPI 仍监听容器内 `8000`，宿主机通过 `8001:8000` 暴露。前端不得直连这些内部服务。
