...
def get_metrics() -> str:
    with _lock:
        ...
        lines += [
            "# HELP zhenhu_patient_cleanups_total 患者出院清理次数",
            "# TYPE zhenhu_patient_cleanups_total counter",
            f"zhenhu_patient_cleanups_total {_patient_cleanups}",
            "# HELP zhenhu_llm_calls_total LLM 调用总次数",
            "# TYPE zhenhu_llm_calls_total counter",
            f"zhenhu_llm_calls_total {_llm_calls}",
            "# HELP zhenhu_llm_errors_total LLM 调用错误次数",
            "# TYPE zhenhu_llm_errors_total counter",
            f"zhenhu_llm_errors_total {_llm_errors}",
            "# HELP zhenhu_llm_cache_hits_total LLM 缓存命中次数",
            "# TYPE zhenhu_llm_cache_hits_total counter",
